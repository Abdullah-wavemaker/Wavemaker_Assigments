...
fetch("http://localhost:8080/RestuarantBilling/invoice")
        .then((response) => response.json())
        .then((data) => {
            var invid = data.length;
            url="http://localhost:8080/RestuarantBilling/invoice/"+invid
            fetch(url)
        .then((response) => response.json())
        .then((data) => {
            inorderidtag.innerHTML=data["order_id"];
            inordertimetag.innerHTML=data["order_time"]
        })
    })
                    ...
                    json.forEach((invoiceItem) => {
                        if(invoiceItem.invoice_id == invid){
                           ...
                           fetch("http://localhost:8080/RestuarantBilling/menu")
                            ...
                            json.forEach((dish) => { 
                                if(dish.id == menuid){
                                    li+= `<div style="display:flex; flex-direction:row; justify-content:space-between"><div>`+dish.dish_name+" X "+quantity+`</div>`+`<div><p>`+dish.price+`</p></div>`+`</div>`
                            ...
                            fetch("http://localhost:8080/RestuarantBilling/taxes")
                            .then((response) => response.json())
                            .then((data) => {
                                var totaltax = 0;
                                data.forEach((tax) => {
                                    totaltax += tax.charge_percentage;
                                })
